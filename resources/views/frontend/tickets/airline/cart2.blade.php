...
<div class="terms-conditions-page container" >
    ...
    <form id="dataFormPage" action="{{ url('airlinee/price-booking') }}" method="POST" >
        ...
        <div class="checkout-box ">
            <div class="row appendSchedule" style="max-height: 500px;overflow-x: scroll;overflow-y: scroll;">
...
@section('js')
    ...
    $(document).on('click','.cekSche',function(e){
        $('#dataFormPage').append(`
          <div class="loadings" >Loading&#8230;</div>
        `);
        if($('#dataFormPage').form('is valid')){
          $('#dataFormPage').ajaxSubmit({
            url:"{{ url('airlinee/get-schedule') }}",
            method:"GET",
            success: function(resp){
              $('.loadings').hide();
