                ...
                <div class="col-md-8">
                    <form action="" method="post">
                        <div class="input-group">
                            <input type="text" id="search-barang" class="form-control" id="inlineFormInputGroupUsername" placeholder="Search..">
                            <div class="input-group-prepend">
                                <button type="button" data-id="{{ $record->id }}" class="btn btn-warning btn-serch">Cari</button>
                            </div>
                        </div>
                    </form>
                </div>
    ...
    <div class="card">
        <div class="card-header">
            <div class="row">
                ...
                <div class="col-md-8">
                    <form action="" method="post">
                        <div class="input-group">
                            <input type="text" id="search" class="form-control" id="inlineFormInputGroupUsername" placeholder="Search..">
                            <div class="input-group-prepend">
        ...
        <div class="card-body">
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                   @foreach ($records as $e)
                        @if ($e->trans_detail->count() > 0)
                            @foreach ($e->trans_detail as $item)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ url('admin/toko/detail',$item->id) }}" class="btn btn-sm"><i class="fas fa-eye"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                        @endif
...
@section('script')
    <script type="text/javascript">
        $(document).on('click','.btn-serch',function(){
            var search = $('#search-barang').val();
            var id = $(this).data('id');
            console.log(id);
            $.ajax({
                type : 'GET',
                url: "{{ url('admin/toko/show/barang') }}",
                data: {search:search,id:id},
                success: function(resp){
                    $('.body-serch').html(resp);
                },
                error: function(resp){
                    $('.body-serch').html('data tidak di temukan');
                }
            });
        });
