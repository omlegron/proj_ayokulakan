					        		...
					        		@if($record->attachments->count() > 0)
						        		...
						        		@if($k==0)
								            <div class="single-product-gallery-item active" id="slide1">
								                <a data-lightbox="image-1" data-title="Gallery" href="{{ imgExist(url('storage/'.$value->url)) }}">
								                    <img class="img-responsive" alt="" src="{{ imgExist(url('storage/'.$value->url)) }}" data-echo="{{ imgExist(url('storage/'.$value->url)) }}" />
								                </a>
								            </div>
								        @else
								        		...
								        		@if($k==0)
										            <div class="col-sm-2"> 
									                    <a class="horizontal-thumb active" data-target="#owl-single-product" data-slide="1" href="#slide1">
									                        <img class="img-responsive" width="85" alt="" src="{{ imgExist(url('storage/'.$value->url)) }}" data-echo="{{ imgExist(url('storage/'.$value->url)) }}" />
									                    </a>
								                    </div>
										        @else
										        	<div class="col-sm-2"> 
												...
												<div class="col-md-1">
													@if($value->user->pictureusers)
	                                            		@if($value->user->pictureusers->count() > 0)
	                                                		...
	                                                		<img src="{{ ($value->user->pictureusers->sortByDesc('created_at')->first()) ? imgExist(url('storage/'.$value->user->pictureusers->sortByDesc('created_at')->first()->url)) : asset('img/users.png') }}" alt="Ayokulakan" style="width: 50px;height: 50px;">
	                                            		@else
