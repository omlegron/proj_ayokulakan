...
@if(isset($record))
@if(isset($record->attachMany))
<div class="card">
	<div class="card-body">
		<div class="row">
			@if($record->attachMany->count() > 0)
			@foreach($record->attachMany as $file)
			<div class="col-md-4">	
				<div class="form-group">
					<center><div class="card" style="width: 25rem;">
						<a href="{{ url('storage/'.$file->url) }}">
							<img class="img-responsive" src="{{ imgExist(url('storage/'.$file->url)) }}" alt="Card image cap" style="height: 165px">
						</a>
					</div></center>
				</div>
			</div>
